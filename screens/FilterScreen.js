...
const FilterScreen = ({ navigation, background = require('../assets/background.png') }) => {
  ...
  useEffect(() => {
    if (!selectedCity) {
      fetch('http://192.168.1.53:3000/establishments/city')
  ...
  const handleSliderClick = () => {
    fetch('http://192.168.1.53:3000/establishments/type')
      .then(response => response.json())
      .then(data => {
        setTypesOfCare(data);
  ...
  const handleSubmit = () => {
    ...
    fetch(`http://192.168.1.53:3000/establishments?city=${encodeURIComponent(criteria.city)}&days=${encodeURIComponent(criteria.days)}&type=${encodeURIComponent(criteria.type)}&children=${encodeURIComponent(criteria.children)}`)
      .then(response => {
        if (!response.ok) {
  ...
  const handleChildrenFetch = () => {
    fetch(`http://192.168.1.53:3000/children?userId=${userId}`)
      .then(response => {
        if (!response.ok) {
          return response.text().then(text => { throw new Error(text) });
        }
